...
export const generateStudentTransactionPDF = (student, transactions) => {
  ...
  doc.text(`Student ID: ${student.id}`, 60, studentInfoY);
  doc.text(`Name: ${student.fullName}`, 60, studentInfoY + 15);
  doc.text(`Email: ${student.email}`, 60, studentInfoY + 30);
  doc.text(`Department: ${student.department?.name || 'N/A'}`, 60, studentInfoY + 45);
  ...
  const totalDue = transactions.reduce((sum, t) => sum + (t.status === 'DUE' ? t.amount : 0), 0);
  ...
  transactions.forEach((transaction, i) => {
    ...
    doc.fillColor(statusColor)
       .text(transaction.status, xPos + 5, rowTop + 10);
  ...
  // Add footer
  doc.fontSize(10)
     .text('This is an official financial statement from the University Management System.', 50, doc.page.height - 50, { align: 'center' });
